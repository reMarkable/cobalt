...
def buildUsageByModuleJs():
  ...
  with openFile(USAGE_BY_MODULE_CSV_FILE_NAME) as csvfile:
...
def buildUsageByCityJs():
  ...
  """
  # Read the data from the csv file and put it into a dictionary.
  with openFile(USAGE_BY_CITY_CSV_FILE_NAME) as csvfile:
    reader = csv.reader(csvfile)
...
def buildUsageByHourJs():
  ...
  """
  # Read the data from the csv file and put it into a dictionary.
  with openFile(USAGE_BY_HOUR_CSV_FILE_NAME) as csvfile:
    reader = csv.reader(csvfile)
...
def main():
  ...
  output_file = os.path.join(OUT_DIR, OUTPUT_JS_FILE_NAME)
