...
bool GcsUtil::Init(const std::string ca_certs_path,
  ...
  http_config_.reset(new HttpTransportLayerConfig);
  http_config_->ResetDefaultTransportFactory(
      new CurlHttpTransportFactory(http_config_.get()));
  http_config_->mutable_default_transport_options()->set_cacerts_path(
      ca_certs_path);

  // Set up OAuth 2.0 flow for a service account.
  googleapis::util::Status status;
  oauth_flow_.reset(
      new OAuth2ServiceAccountFlow(http_config_->NewDefaultTransport(&status)));
  if (!status.ok()) {
  ...
  oauth_flow_->InitFromJson(json);
  oauth_flow_->set_default_scopes(
      StorageService::SCOPES::DEVSTORAGE_READ_WRITE);
  // Connect the credential with the AuthFlow.
  oauth_credential_.set_flow(oauth_flow_.get());

  // Construct the storage service.
  storage_service_.reset(
      new StorageService(http_config_->NewDefaultTransport(&status)));
  if (!status.ok()) {
    LOG(ERROR) << "GcsUitl::Init(). Error creating new Http transport: "
               << status.ToString();
    return false;
...
bool GcsUtil::Ping(std::string project_id) {
  ...
  google_storage_api::BucketsResource_ListMethod request(
      storage_service_.get(), &oauth_credential_, project_id);
  Json::Value value;
